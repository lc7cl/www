				  ...
				  int recycle)
{
	...
	if(recycle) {
		result->recycle_bin = (recycle_elem**)allocator(sizeof(struct recycle_elem*)
...
region_alloc(region_type *region, size_t size)
{
	...
	if (aligned_size >= region->large_object_size) {
		result = (char*)region->allocator(size + sizeof(struct large_elem));
		if (!result)
			return NULL;
		...
		return (void *)(result + sizeof(struct large_elem));
	}

	if (region->recycle_bin && region->recycle_bin[aligned_size]) {
		result = (char*)region->recycle_bin[aligned_size];
